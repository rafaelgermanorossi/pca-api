...
public class SybaseDBManager extends AbstractDBManager {
		
	private SybaseDBDAO dao;
	
	public SybaseDBManager(ConfigBean config) throws Exception{
		super(config);
		dao = new SybaseDBDAO(config);
	}
	...
	public void updateIncrementalQueryJava(QueryBean query) throws SQLException{
		...
		String sql = "select * from "+query.getViewName()+" where "+query.getIdentityColumn()+
				" > convert("+query.getIdentityColumnType()+","+query.getCurrentIdentityMaxValue()+") order by "+query.getIdentityColumn();
